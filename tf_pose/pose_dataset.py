...
class CocoMetadata:
    ...
    def parse_floats(four_nps, adjust=0):
        ...
        return [
            (CocoMetadata.parse_float(four_nps[x * 4 : x * 4 + 4]) + adjust)
            for x in range(len(four_nps) // 4)
        ]
    ...
    def __init__(self, idx, img_url, img_meta, annotations, sigma):
        ...
        for ann in annotations:
            ...
            joint_list.append(
                [(x, y) if v >= 1 else (-1000, -1000) for x, y, v in zip(xs, ys, vs)]
            )
    ...
    def get_heatmap(self, target_size):
        heatmap = np.zeros(
            (CocoMetadata.__coco_parts, self.height, self.width), dtype=np.float32
        )
    ...
    def get_vectormap(self, target_size):
        vectormap = np.zeros(
            (CocoMetadata.__coco_parts * 2, self.height, self.width), dtype=np.float32
        )
        countmap = np.zeros(
            (CocoMetadata.__coco_parts, self.height, self.width), dtype=np.int16
        )
        for joints in self.joint_list:
            for plane_idx, (j_idx1, j_idx2) in enumerate(CocoMetadata.__coco_vecs):
                ...
                if (
                    center_from[0] < -100
                    or center_from[1] < -100
                    or center_to[0] < -100
                    or center_to[1] < -100
                ):
                    continue

                CocoMetadata.put_vectormap(
                    vectormap, countmap, plane_idx, center_from, center_to
                )
    ...
    @jit(nopython=True)
    def put_vectormap(
        vectormap, countmap, plane_idx, center_from, center_to, threshold=8
    ):
...
class CocoPose(RNGDataFlow):
    ...
    def display_image(inp, heatmap, vectmap, as_numpy=False):
        ...
        a.set_title("Heatmap")
        plt.imshow(
            CocoPose.get_bgimg(inp, target_size=(heatmap.shape[1], heatmap.shape[0])),
            alpha=0.5,
        )
        ...
        plt.imshow(
            CocoPose.get_bgimg(inp, target_size=(vectmap.shape[1], vectmap.shape[0])),
            alpha=0.5,
        )
    ...
    def __init__(
        self, path, img_path=None, is_train=True, decode_img=True, only_idx=-1
    ):
        ...
        self.img_path = (img_path if img_path is not None else "") + (
            "train2017/" if is_train else "val2017/"
        )
            ...
            for _ in range(10):
                try:
                    ...
                    if resp.status_code // 100 != 2:
                        logger.warning(
                            "request failed code=%d url=%s"
                            % (resp.status_code, meta.img_url)
                        )
                        time.sleep(1.0)
                        continue
                    ...
                    logger.warning(
                        "request failed url={}, err={}".format(meta.img_url, str(e))
                    )
        else:
...
def get_dataflow(path, is_train, img_path=None):
    ...
    if is_train:
        ...
        ds = MultiThreadMapData(
            ds, nr_thread=16, map_func=read_image_url, buffer_size=1000
        )
...
class DataFlowToQueue(threading.Thread):
    ...
    def default_sess(self):
        if self._sess:
            ...
            logger.warning(
                f"DataFlowToQueue {self.name} wasn't under a default session!"
            )
            yield
            ...
            try:
                while not self._coord.should_stop():
                    try:
                        ...
                        logger.error(f"err type1, placeholders={self.placeholders}")
                        sys.exit(-1)
                    except Exception as e:
                        logger.error(
                            "err type2, err={}, placeholders={}".format(
                                str(e), self.placeholders
                            )
                        )
                        if isinstance(e, RuntimeError) and "closed Session" in str(e):
                            ...
                            logger.exception(
                                "Exception in {}:{}".format(self.name, str(e))
                            )
                        sys.exit(-1)
            except Exception as e:
...
if __name__ == "__main__":
    ...
    df = _get_dataflow_onlyread(
        "/data/public/rw/coco/annotations", True, "/data/public/rw/coco/"
    )
    ...
    with tf.Session() as sess:
        ...
        for idx, dp in enumerate(df.get_data()):
            if idx == 0:
                for d in dp:
                    logger.info(f"%d dp shape={d.shape}")
            print(time.time() - t1)
            t1 = time.time()
            CocoPose.display_image(
                dp[0], dp[1].astype(np.float32), dp[2].astype(np.float32)
            )
