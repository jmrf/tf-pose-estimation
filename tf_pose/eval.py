...
def write_coco_json(human, image_w, image_h):
    ...
    for coco_id in coco_ids:
        ...
        keypoints.extend(
            [round_int(body_part.x * image_w), round_int(body_part.y * image_h), 2]
        )
    return keypoints
...
if __name__ == "__main__":
    ...
    coco_json_file = (
        args.coco_dir + "annotations/person_keypoints_val%s.json" % args.cocoyear
    )
    ...
    write_json = "../etcs/{}_{}_{:0.1f}.json".format(
        args.model, args.resize, args.resize_out_ratio,
    )

    logger.debug(
        "initialization {} : {}".format(args.model, get_graph_path(args.model))
    )
    ...
    for i, k in enumerate(tqdm_keys):
        ...
        for human in humans:
            item = {
                "image_id": img_idx,
                "category_id": 1,
                "keypoints": write_coco_json(
                    human, img_meta["width"], img_meta["height"]
                ),
                "score": human.score,
            }
            result.append(item)
        ...
        tqdm_keys.set_postfix(
            OrderedDict({"inference time": elapsed, "score": avg_score})
        )
        if args.data_idx >= 0:
